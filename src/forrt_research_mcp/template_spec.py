...
VENDORED from ScienceLiveHub/forrt-replication-template (`scripts/template_fields.py`,
MIT, same authors) so this server can parse a template without depending on a
study repo. Keep the two in sync; the intent is that the template repo
eventually imports this copy rather than keeping its own.
...
`templates.py`, so this stays offline-testable.

Dependency: `rdflib`. Parsing RDF with a regex is exactly the hand-rolled
fragility this tier exists to remove, so we parse it as RDF.
"""
